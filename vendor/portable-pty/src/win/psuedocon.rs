...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        let base = conpty_base_flags();
        // Win11 22H2+ passthrough forwards ANSI RGB to the host without palette reduction.
        let con = match try_create_conpty(
            size,
            &input,
            &output,
            base | PSEUDOCONSOLE_PASSTHROUGH_MODE,
        ) {
            Ok(con) => con,
            Err(_) => try_create_conpty(size, &input, &output, base).map_err(|result| {
                anyhow::anyhow!("failed to create psuedo console: HRESULT {}", result)
            })?,
        };
        Ok(Self { con })
    }
